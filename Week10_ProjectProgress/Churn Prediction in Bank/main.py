...
def predict():
    if request.method == 'POST':
        ...
        prediction = model.predict([[CreditScore, Age, Tenure, Balance, NumOfProducts, HasCrCard, IsActiveMember,
                                     EstimatedSalary,Geography_France, Geography_Germany, Geography_Spain, Gender_Male]])
        if prediction == 1:
            return render_template('customerchurn.html', prediction_text="The Customer will leave the bank")
        else:
            return render_template('customerchurn.html', prediction_text="The Customer will not leave the bank")
